...
import { Loader2, ArrowUpRight, Wallet, History, AlertCircle } from "lucide-react";
...
export default function FinancePage() {
  ...
  useEffect(() => {
    ...
    // 1. Realtime Wallet
    const unsubWallet = onSnapshot(doc(db, "stores", storeId, "wallet", "main"), (doc) => {
      if (doc.exists()) {
        setWallet(doc.data());
      } else {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (result.data.success) {
        setMessage({ type: "success", text: "Withdrawal Initiated! Funds are on the way." });
              ...
              <div key={tx.id} className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
                <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        ...
                        tx.type === 'debit' || tx.type === 'payout' ? 'bg-red-100 text-red-600' : 'bg-gray-100'
                    }`}>
                        {tx.type === 'credit' ? <ArrowUpRight className="rotate-180" size={18} /> : <ArrowUpRight size={18} />}
                    </div>
                    <div>
        ...
        {showWithdraw && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
             ...
             >
                ...
                <div className="space-y-4">
                    <div>
                        <label className="text-sm font-bold block mb-2">Amount (GHS)</label>
                        <input 
                            type="number" 
                            value={amount}
                    ...
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex gap-3 text-blue-800 text-sm">
                        ...
                        <p>Transfers are processed instantly via Paystack. Charges may apply.</p>
                    </div>
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button 
                            ...
                            disabled={processing || !amount || parseFloat(amount) > wallet.currentBalance}
                            className="flex-1 py-4 bg-black text-white rounded-xl font-bold hover:scale-105 transition-transform disabled:opacity-50"
                        >
                            {processing ? <Loader2 className="animate-spin mx-auto" /> : "Confirm"}
                        </button>
                    </div>
                </div>
             </motion.div>
